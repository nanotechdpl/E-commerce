...
import {
  ...
  setEmployers,
  Employer,
} from "@/redux/fetures/service/employersSlice";
...
const CreateEmployee: React.FC = () => {
  ...
  const employers = useSelector(
    (state: RootState) => state.employers.employers
  );
  ...
  const handleEdit = (id: string) => {
    const row = employers.find((employer) => employer._id === id);
    if (row) {
      setFormData(row);
      setIsEdit(true);
  ...
  const handleDeleteRow = async (id: string) => {
      try {
      const res = await instance.delete(`/employers/${id}`);
  ...
  const handleShowModal = (title: string) => {
    ...
    try {
      const res = await instance.put(`/employers/status/${id}`, {
        status,
      });
      if (res.data && res.data.success) {
        // Update the visa in Redux store
        dispatch(updateEmployer(res.data.data));
      ...
      toast.error(
        error?.response?.data?.message || "Error updating Employee status"
      );
    }
  };
  ...
  const handleAddOrEditRow = async () => {
    ...
    if (isEdit) {
      try {
        const res = await instance.put(`/employers/${formData._id}`, formData);
      ...
      try {
        ...
        dispatch(setEmployers(res.data.data)); // Replace all, or use addEmployer with the last one
        setFormData({
          photo: "",
          name: "",
          title: "",
    ...
    (async () => {
      try {
        const res = await instance.get(
          `/employers/allEmployees?query=${query}&limit=${limit}`
        );

        console.log("kkkkkkk", res);

        if (res.data && res.data.success) {
          dispatch(setEmployers(res.data.data));
      ...
      <div className="p-4 mt-[0rem]">
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                ...
                {employers && employers?.length > 0 ? (
                  employers?.map((row: any, index: any) => (
                    <tr
                      key={index}
                      className={`${
        ...
        <div className="flex flex-col w-full items-center justify-center my-7 gap-5">
          <p className="font-inter font-semibold text-base leading-[19.36px] text-black-4">
            Showing {employers?.length || 0} of {totalData} results
          </p>
