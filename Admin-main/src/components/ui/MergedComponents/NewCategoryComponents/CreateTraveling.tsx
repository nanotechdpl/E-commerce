...
import CreateTravelingForm from "./CreateTravelingForm";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "@/redux/store/store";
import { addTraveling, deleteTraveling, setTravelings, Traveling, updateTraveling } from "@/redux/fetures/service/travelingSlice";
import instance from "@/api/axios";
import { toast } from "react-toastify";
import { PiToggleRight } from "react-icons/pi";


const CreateTraveling = () => {
  ...
  const travelings = useSelector((state: RootState) => state.traveling.travelings);
  const dispatch = useDispatch();

   const [formData, setFormData] = useState<Traveling>({
    photo: '',
    description: "",
    title: "",
   category: "",
  isDeleted: false,
    ...
    try {
      const res = await instance.put(`/menu-services/traveling/status/${id}`, {
        status,
      });
      if (res.data && res.data.success) {
        // Update the visa in Redux store
        dispatch(updateTraveling(res.data.data));
        toast.success("Traveling status updated successfully!");
      } else {
        toast.error(res.data?.message || "Failed to update traveling status");
      }
    } catch (error: any) {
      toast.error(
        error?.response?.data?.message || "Error updating traveling status"
      );
    }
  };
    ...
    if (isEdit) {
      try {
        const res = await instance.put(
          `/menu-services/traveling/${formData._id}`,
          formData
        );
  ...
  isDeleted: false,
      ...
      try {
        const res = await instance.post("/menu-services/traveling", formData);
        if (res.data && res.data.success) {
         dispatch(addTraveling(res.data.data));
          setFormData({
              photo: '',
        description: "",
        title: "",
  ...
  const handleEdit = (id: string): void => {
    const row = travelings.find((traveling) => traveling._id === id);
    if (row) {
      setFormData(row);
      setIsEdit(true);
  ...
  const handleDeleteRow = async (id: string) => {
    try {
      const res = await instance.delete(`/menu-services/traveling/${id}`);
      if (res.data && res.data.success) {
        dispatch(deleteTraveling(id));
    ...
    (async () => {
      try {
        const res = await instance.get(
          `/menu-services/traveling?query=${query}&limit=${limit}`
        );

        if (res.data && res.data.success) {
           dispatch(setTravelings(res.data.data));
          console.log(res.data, "res.data traveling");
          setTotalData(res.data.totalData);
        }
      } catch (error) {
        console.log("Error fetching visas", error);
        toast.error("Error fetching Traveling data");
      }
    })();
    ...
    <div className="p-2">
      ...
      <div className="flex flex-col w-full items-center justify-center my-7 gap-5">
        <p className="font-inter font-semibold text-base leading-[19.36px] text-black-4">
          Showing {travelings?.length || 0} of {totalData} results
        </p>
...
export default CreateTraveling;
