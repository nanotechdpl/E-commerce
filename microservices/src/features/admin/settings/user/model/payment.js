...
const { paymentModel } = require("../../../../users/core/db/payment");
const {
  paymentstatuslogModel,
} = require("../../../../users/core/db/paymentstatuslog");
...
const adminuserpaymentdashboardModel = async (data, res) => {
  try {
    ...
    // }
    userpayments = await paymentModel.find().populate({
      path: "bankid",  
      select: "name", 
    }).populate({
      path: "userid",  
...
const adminretrievesinglepaymentModel = async (data, res) => {
  try {
    ...
    const statuslog = await paymentstatuslogModel.find({ paymentid }).find({ orderid : payment.orderid })
